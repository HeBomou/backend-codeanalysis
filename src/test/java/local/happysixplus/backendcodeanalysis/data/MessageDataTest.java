...
class MessageDataTest {
    ...
    void init() {
        data.deleteAll();
        messagePo = new MessagePo(1234L, 1L, 2L, "SKT", "Faker");
    }
    ...
    public void testUpdate() {
        messagePo = data.save(messagePo);
        messagePo = new MessagePo(messagePo.getId(), 3L, 4L, "RNG", "Uzi");
        messagePo = data.save(messagePo);
        MessagePo resPo = data.findById(messagePo.getId()).get();
        assertEquals(resPo.getId(), messagePo.getId());
        assertEquals(resPo.getSenderId(), messagePo.getSenderId());
    ...
    public void testFindBySenderId() {
        messagePo = data.save(messagePo);
        MessagePo messagePo1 = data.save(new MessagePo(12345L, 1L, 3L, "SKT", "otto"));
        MessagePo messagePo2 = data.save(new MessagePo(12346L, 1L, 4L, "RNG", "Xiaohu"));
